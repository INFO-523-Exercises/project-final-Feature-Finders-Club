...
-   Relationship between headlines and Apple's stock price

-   Sentiment analysis

-   Provide valuable information for understanding stock price movements
...
1.  Loaded necessary packages for data manipulation, visualization, sentiment analysis, and financial data retrieval.

2.  Extracted data (yfinance library and News API) for the last 30 days.

3.  Transformed data (sorting, filtering, removing duplicates).

4.  Sentiment analysis using the VADER sentiment analysis tool.
...
::: incremental
[ - In this project, we delved into the dynamic intersection of finance and sentiment analysis, specifically focusing on Apple stock. Leveraging the `SentimentIntensityAnalyzer` function in the `vaderSentiment` library in Python, we embarked on a journey to explore the emotional tone surrounding Apple's stock based on textual data. ]{style="font-size:23px;"}

[ - The project initiated with data acquisition, utilizing the `yfinance` library to fetch comprehensive stock data through API calls. Subsequently, sentiment analysis was performed using `vaderSentiment`, a powerful tool for gauging sentiment intensity in text data. The sentiment scores obtained were then plotted against the `Apple` stock price, creating a visual representation of the relationship between sentiment and market dynamics. ]{style="font-size:23px;"}

[ - The comparison plot revealed intriguing insights into the potential correlation between sentiment shifts and stock price movements. Peaks and troughs in sentiment scores corresponded with noteworthy fluctuations in the stock chart, indicating a potential influence of public sentiment on market trends. ]{style="font-size:23px;"}
:::

## Conclusion

::: incremental
[ - However, it's essential to acknowledge the multifaceted nature of stock markets and the intricate interplay of various factors influencing stock prices. While sentiment analysis provides valuable insights, it should be viewed as one of many tools in the broader landscape of financial analysis. ]{style="font-size:23px;"}

[ - In conclusion, this project exemplifies the fusion of sentiment analysis techniques with financial data to derive meaningful insights into the emotional undercurrents surrounding Apple stock. As we navigate the complex realm of stock markets, this approach adds a nuanced layer to our understanding, paving the way for further exploration and refinement of strategies in financial decision-making. ]{style="font-size:23px;"}
:::

## Challenges faced

::: incremental
[ ]{style="font-size:23px;"}[ - One of the limitation to this project was fetching data from a live data source. We used newsapi.org to fetch realtime news but this platform only lets us fetch data for the past 30 days which would stop us from training the model on a wider timescale. ]{style="font-size:23px;"}
